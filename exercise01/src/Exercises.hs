...
myTree :: HTree (Branch (Branch Empty Integer Empty) String (Branch Empty Integer Empty))
myTree = HBranch left "Dog" right where
...
foldValues aList = foldG start aList where
    start = (mempty, mempty)
...
parse dirty = let 
  niceExpr = clean dirty

  clean (DirtyIntValue i)  = IntValue i
  clean (DirtyBoolValue b) = BoolValue b
  clean (DirtyAdd a b)     = Add (clean a) (clean b)
  clean (DirtyEquals a b)  = Equals (clean a) (clean b)
  
              in case niceExpr of 
                (IntValue i) -> Just (IntValue i)
                _            -> Nothing
